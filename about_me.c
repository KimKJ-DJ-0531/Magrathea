...
int main(void)
{
    ...
    struct Member Arthur_Dent = {"아서 덴트", "남", "기계 공학", 15, "아키텍트","C/C++, 자바, 스프링, 파이썬, 머신러닝/딥러닝", "실리콘 밸리에서 SNS, 핀테크, HR, 미디어 등의 다양한 프로젝트에 참여하며 경력을 쌓았습니다. 성공과 실패를 겪으며 얻게 된 깨달음을 여러분과 나누고 싶습니다."};
    ...
    struct Member Seo_Hae_Gyeong = {"서혜경", "여", "정치외교", 0, "개발자", "파이썬, 스위프트, 코틀린, Node.js, 피그마", "학부과정에서 과제로 프로젝트는 많이 진행해봤지만, 기업의 사활이 걸린 실전 프로젝트는 제 인생의 첫 경험입니다. 이 프로젝트에 참여하게 되어 기대도 많이 되는 한편으로, 제가 잘할 수 있을까 염려가 되기도 합니다. 마그라테아 프로젝트에 관해 들었을 때 제 속에서 이 프로젝트는 인생을 걸고 싶다는 생각을 했습니다. 링크드인을 통해 이미 아서님의 명성을 접했던 터라, 이번 프로젝트에 함께할 수 있어서 영광입니다."};
    ...
    for (int i = 0; i < 10; i++)
    {
        if(members[i].name == NULL) continue;
